...
- **Local Kafka module**: Maven-launched single-node Apache Kafka 4.3.1 in KRaft mode; no Docker or Kafka installation required.
...
## Start Kafka - no Docker required

The repository now contains a `local-kafka` module. It launches a single-node
Apache Kafka 4.3.1 broker in KRaft mode using the Kafka runtime dependencies
downloaded by Maven. No Docker, ZooKeeper, Git Bash, WSL, or separate Kafka
...
The first run automatically formats its KRaft storage. Later runs reuse the
same storage under `./data/kafka`. Configuration is in:
...
CMS H2 TCP           9093
Kafka controller     9094
```

CMS already points at `localhost:9092`, so no CMS configuration change is
needed. If an organisation-provided Kafka broker is available instead, simply
do not start `local-kafka` and change `spring.kafka.bootstrap-servers` in the
CMS `application.properties`.
...
- JDK 21
- Maven 3.9+
- Maven access to download the local Kafka runtime dependencies on the first build
...
Start local Kafka first (terminal 1):

```bash
mvn -pl local-kafka spring-boot:run
```

Start the rule engine next (terminal 2):

```bash
mvn -pl rule-engine-service spring-boot:run
```

Then start CMS (terminal 3):
